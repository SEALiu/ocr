...
class Decoder:
    def __init__(self, table, blank_index=None, merge_repeated=True):
        """
    ...
    def map2string(self, inputs):    
        ...
        for indexes in inputs:
            text = [self.table[char_index] for char_index in indexes if char_index != self.blank_index and char_index < len(self.table)]
    ...
    def decode(self, inputs, from_pred=True, method='greedy'):
        if from_pred:
            ...
            if method == 'greedy':
                ...
                decoded, _ = tf.nn.ctc_beam_search_decoder(inputs=tf.transpose(inputs, perm=[1, 0, 2]), sequence_length=logit_length)
            
            inputs = decoded[0]
            
        decoded = tf.sparse.to_dense(inputs, default_value=self.blank_index).numpy()
