...
class BatchNormalization(L.BatchNormalization):
    """
    Identical to L.BatchNormalization, but adds the option to freeze parameters.
    """
...
class ResNet(tf.keras.Model):
    """
        ...
        :return models: ResNet models with encoding output (if `include_top=False`) or classification output (if `include_top=True`)
        Usage:
        """
    ...
    def __init__(
        ...
        x = L.Conv2D(64, (7, 7), strides=(2, 2), use_bias=False, name="conv1", padding="same")(inputs)
        x = BatchNormalization(axis=axis, epsilon=1e-5, freeze=freeze_bn, name="bn_conv1")(x)
        x = L.Activation("relu", name="conv1_relu")(x)
        x = L.MaxPooling2D((3, 3), strides=(2, 2), padding="same", name="pool1")(x)
...
def resnet_basic(
    ...
    def f(x):
        y = L.ZeroPadding2D(padding=1, name="padding{}{}_branch2a".format(stage_char, block_char))(x)

        y = L.Conv2D(filters, kernel_size, strides=stride, use_bias=False, name="res{}{}_branch2a".format(stage_char, block_char), **parameters)(y)

        y = BatchNormalization(axis=axis, epsilon=1e-5, freeze=freeze_bn, name="bn{}{}_branch2a".format(stage_char, block_char))(y)

        y = L.Activation("relu", name="res{}{}_branch2a_relu".format(stage_char, block_char))(y)

        y = L.ZeroPadding2D(padding=1, name="padding{}{}_branch2b".format(stage_char, block_char))(y)

        y = L.Conv2D(filters, kernel_size, use_bias=False, name="res{}{}_branch2b".format(stage_char, block_char), **parameters)(y)

        y = BatchNormalization(axis=axis, epsilon=1e-5, freeze=freeze_bn, name="bn{}{}_branch2b".format(stage_char, block_char))(y)

        if block == 0:
            shortcut = L.Conv2D(filters, (1, 1), strides=stride, use_bias=False, name="res{}{}_branch1".format(stage_char, block_char), **parameters)(x)
        ...
        y = L.Add(name="res{}{}".format(stage_char, block_char))([y, shortcut])

        y = L.Activation("relu", name="res{}{}_relu".format(stage_char, block_char))(y)
...
def resnet_bottleneck(
    ...
    def f(x):
        y = L.Conv2D(filters, (1, 1), strides=stride, use_bias=False, name="res{}{}_branch2a".format(stage_char, block_char), **parameters)(x)
        ...
        y = L.Activation("relu", name="res{}{}_branch2b_relu".format(stage_char, block_char))(y)

        y = L.Conv2D(filters * 4, (1, 1), use_bias=False, name="res{}{}_branch2c".format(stage_char, block_char), **parameters)(y)

        y = BatchNormalization(axis=axis, epsilon=1e-5, freeze=freeze_bn, name="bn{}{}_branch2c".format(stage_char, block_char))(y)

        if block == 0:
            shortcut = L.Conv2D(filters * 4, (1, 1), strides=stride, use_bias=False, name="res{}{}_branch1".format(stage_char, block_char), **parameters)(x)
